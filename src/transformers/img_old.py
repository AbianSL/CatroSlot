...
class OldImageTransform(ImageTransform):
    def __init__(
        ...
        self._small_non_symbol = self._small_non_symbol.copy().resize(
            (39, 39), resample=Image.BILINEAR
        )
        self._defend_img = self._defend_img.resize((40, 39), resample=Image.BILINEAR)
        self._life_img = self._life_img.resize((39, 39), resample=Image.BILINEAR)
        self._power_img = self._power_img.resize((39, 39), resample=Image.BILINEAR)
